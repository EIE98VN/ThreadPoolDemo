...
public class BlockingQueue<Type> {
	...
	public synchronized void enqueue(Type task) throws InterruptedException {
		int activeThread = 0;
		for(int i=0;i< TestThreadPool.maxThread;i++) {
			if(ThreadPool.threadList.get(i).isAlive()) activeThread++;
		}
		while (this.queue.size() == this.MAX_TASK_IN_QUEUE) {
			if(activeThread < TestThreadPool.maxThread ) {
				if(this.queue.size() == this.MAX_TASK_IN_QUEUE) {
					ThreadPool.threadList.get(activeThread).start();
				}
			}
			if(this.queue.size() == this.MAX_TASK_IN_QUEUE) wait();
		}
